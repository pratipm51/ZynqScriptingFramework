...
> [!NOTE]
> **Architecture Clarification (`top.vhd` Master vs `system` BD Subsystem):**
> Clicking **"Sync to Framework"** in Vivado automatically syncs the Block Design wrapper directly to **`src/hdl/system.vhd`** (retaining entity name `system`).
> `src/hdl/top.vhd` is your permanent master top-level module and instantiates the BD wrapper directly using VHDL entity instantiation (`system_i : entity work.system`).
> Because `top.vhd` uses direct entity instantiation (`entity work.system`), VHDL automatically binds to the updated `system.vhd` generated by Vivado whenever Block Design ports change. You never have to manually copy component port declarations or merge `.new` files!
